# Running the ingest every 3 hours on Windows (autonomous)

**Most efficient:** use **Windows Task Scheduler**. No extra process runs between runs; the task simply triggers every 3 hours. No Node daemon, no cron service—just a scheduled task.
...
That creates the scheduled task **AnimalMind Ingest** to run every 3 hours. If you get "access denied," right‑click the script → **Run as administrator**. To run the task now: `schtasks /run /tn "AnimalMind Ingest"`. To remove it: `schtasks /delete /tn "AnimalMind Ingest" /f`.
...
3. **General**
  ...
  - Description: `Data ingest every 3 hours (PubMed + CDC) for animal health insights`
...
4. **Triggers**
   - New → **On a schedule**
  - Settings: **Daily**, repeat every **3 hours**, for a duration of **Indefinitely**
  - Or: **Repeat task every**: 3 hours, for duration **Indefinitely**
...
- **Schedule:** The task **AnimalMind Ingest** runs **every 3 hours** (on the hour), as set by `scripts\setup-hourly-task.cmd` (`/sc hourly /mo 3 /st 00:00`).
...
- **Efficient:** No process running between runs; OS wakes the task every 3 hours.
...
- **To run after wake:** In the task **Settings** tab, enable **"Run task as soon as possible after a scheduled start is missed."** Then when you wake the PC, the task will run once for the missed time (and then resume the normal 3-hour schedule).
...
- **Balanced freshness:** Surveillance (CDC) and literature (PubMed) updates appear multiple times per day without running continuously.
- **Same VM cost:** The droplet is billed per month, not per run; running every 3 hours does not increase cost.
