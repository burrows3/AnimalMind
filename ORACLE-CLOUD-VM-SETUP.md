...
Step-by-step below: create an **Always Free** VM, install Node.js, clone the repo, and run **ingest + think + push** every 12 hours so your PC can stay off.
...
That’s it. Ingest is scheduled every 12 hours; optionally the script starts the dashboard with PM2. Full details below.
...
## 8. Schedule ingest every 12 hours (cron)
...
If asked, choose an editor (e.g. `nano`). Add this line (runs at 00:00 and 12:00):

```
0 */12 * * * cd ~/AnimalMind && /usr/bin/node scripts/ingest-data-sources.js && /usr/bin/node scripts/think-autonomous.js && /usr/bin/node scripts/push-ingest-to-github.js >> ~/AnimalMind/memory/cron.log 2>&1
```
...
```
0 */12 * * * ~/AnimalMind/scripts/run-ingest.sh >> ~/AnimalMind/memory/cron.log 2>&1
```

Verify: `crontab -l` should show the line. After 12 hours, check `memory/cron.log` (and `memory/ingest.log` if you use the script) and GitHub for a new commit.
...
| 8 | Added a cron job to run ingest + think + push every 12 hours |
| 9 | (Optional) Started the dashboard with PM2 on port 3000 |

After this, **your PC can stay off**. The Oracle VM runs ingest + think + push every 12 hours and can serve the dashboard at **http://YOUR_VM_PUBLIC_IP:3000**.
