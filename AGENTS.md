...
- **Windows:** Scheduled task (every 6 hours) runs `run-ingest.cmd`, which runs ingest → think → surveillance reviewer → literature reviewer → synthesizer → push.
- **Linux/VM:** Cron runs `run-ingest.sh` every 6 hours the same way.
