...
When prompted, paste your **GitHub Personal Access Token** ([github.com/settings/tokens](https://github.com/settings/tokens), `repo` scope). Ingest will be scheduled every 6 hours; optionally start the dashboard with PM2.
...
Paste your **GitHub token** when asked. Ingest runs every 6 hours; say **y** to PM2 if you want the dashboard at `http://YOUR_DROPLET_IP:3000`. Open port **3000** in DigitalOcean **Firewall** if you want to access it from the web.
...
- **Ingest:** Runs every 6 hours via cron (ingest → think → agents → push to GitHub).
...
### Switch existing VM from 12-hour to 6-hour

If the VM was set up earlier with every-12-hour cron, update it:

```bash
crontab -e
```

Replace the line that has `*/12` with:
