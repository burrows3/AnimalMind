...
function main() {
  ...
  lines.push('');
  lines.push('## Autonomous-agent topics (frontier)', '');
  lines.push('Each scheduled run finds literature for these frontier topics (PubMed queries). Agents and the network are oriented toward these hard questions:');
  lines.push('');
  lines.push('- Understanding how to communicate with animals');
  lines.push('- Animal sentience and cognition');
  lines.push('- Cross-species welfare at scale');
  lines.push('- Decoding animal pain and distress');
  lines.push('- One-health intelligence (human–animal–environment)');
  lines.push('- Early detection of disease across species');
  lines.push('- Animal minds: what they know and feel');
  lines.push('');
  lines.push('Topic results appear under **Literature** in the data (grouped by topic).');
  lines.push('');
