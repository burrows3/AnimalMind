...
3. **Cron:** Run the ingest every 12 hours, e.g.  
   `0 */3 * * * cd ~/AnimalMind && node scripts/ingest-data-sources.js && node scripts/think-autonomous.js && node scripts/push-ingest-to-github.js`
...
Once this is set up, **your PC can be off**; the VM runs ingest + think + push every 12 hours and can serve the dashboard 24/7.
