...
/** Extract from heading until next H2 (##), so H3 subsections are included. */
function extractSectionUntilH2(text, headingRegex) {
  if (!text) return null;
  const lines = text.split(/\r?\n/);
  let start = -1;
  for (let i = 0; i < lines.length; i += 1) {
...
/** Try multiple heading patterns; return first non-null. Truncate to maxChars for UI. */
function extractSectionWithFallbacks(text, patterns, maxChars = 5000) {
  if (!text) return null;
  for (const re of patterns) {
    const section = extractSection(text, re);
    if (section && section.length > 0) {
      return maxChars && section.length > maxChars
...
function getAgentReasoning() {
  ...
  const surveillance = {
    ...
    reasoning: extractSectionWithFallbacks(surveillanceText, [
      /^#{2,6}\s+LLM reasoning\s+\(NVIDIA\)/i,
    ]),
  };
  const literature = {
    lastRun: extractLastRun(literatureText),
    reasoning: extractSectionWithFallbacks(literatureText, [
      /^#{2,6}\s+LLM reasoning\s+\(NVIDIA\)/i,
      /^#{2,6}\s+Reasoning by autonomous-agent topic\s*$/i,
    ]),
  };
  let synthesisReasoning = extractSection(synthesisText, /^#{2,6}\s+LLM synthesis\s+\(NVIDIA\)/i);
  if (!synthesisReasoning || synthesisReasoning.length === 0) {
    synthesisReasoning = extractSectionUntilH2(synthesisText, /^##\s+Opportunities\s+\(from agent thinking\)\s*$/i);
    if (synthesisReasoning && synthesisReasoning.length > 5000) {
      synthesisReasoning = synthesisReasoning.slice(0, 5000) + '\n\n— truncated for display —';
    }
  }
  const synthesis = {
