...
- **Autonomous research** – Agents run periodically (e.g. data ingest every 6 hours, heartbeat) to collect data and surface **insights into animal health risks, opportunities, and partnerships**.
...
Run **once:** `scripts\setup-hourly-task.cmd` — that creates a Windows scheduled task so the ingest runs every 6 hours. See [SCHEDULE-WINDOWS.md](./SCHEDULE-WINDOWS.md). Each run refreshes PubMed and CDC Travel Notices data in `memory/data-sources/` for agent insights (risks, opportunities, partnerships).
