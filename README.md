...
- **Autonomous research** – Agents run periodically (data ingest every 3 hours) to collect data and surface **insights into animal health risks, opportunities, and partnerships**.
...
Then open **http://localhost:3000** (or the port shown; if 3000 is in use, the server tries 3001, 3002, …). The dashboard is in **`public/`**—run `npm start` from the repo root. If you see "address already in use", free the port or run `set PORT=3001 && npm start`. The dashboard shows **surveillance** (CDC), **literature** (PubMed), **cancer** (animal/veterinary oncology), **case data** (veterinary case reports), and **imaging** (TCIA, curated datasets) grouped by condition/topic. Data is refreshed every 3 hours.
...
## Autonomous run every 3 hours (Windows)

Run **once:** `scripts\setup-hourly-task.cmd` — that creates a Windows scheduled task so the ingest runs every 3 hours. See [SCHEDULE-WINDOWS.md](./SCHEDULE-WINDOWS.md). Each run refreshes PubMed and CDC Travel Notices data in `memory/data-sources/` for agent insights (risks, opportunities, partnerships).
