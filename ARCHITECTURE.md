...
**Flow (every 12 hours when scheduled):**  
...
When `run-ingest.cmd` or `run-ingest.sh` runs (e.g. every 12 hours):
...
| Task | What runs | Goal |
|------|-----------|------|
| **Collect data** | Ingest every 12 hours (PubMed, CDC, cancer, case reports, curated, TCIA) | Literature, surveillance, cancer, case data, and imaging in `memory/data-sources/` and DB. |
...
The goal is to run **autonomously**: data collection on a schedule (e.g. every 12 hours on Windows via Task Scheduler), then agents (or you) use that data for insights into risks, opportunities, and partnerships.

### How to run the ingest (VM job)

- **Manual:** `npm install` then `npm run ingest`.
- **Windows, every 12 hours (autonomous):** Use Task Scheduler; see [SCHEDULE-WINDOWS.md](./SCHEDULE-WINDOWS.md).
