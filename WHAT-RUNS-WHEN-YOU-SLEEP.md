...
|-----------|--------------|--------------|
| **AnimalMind Ingest** (Windows scheduled task) | **Every 12 hours** (if you ran `scripts\setup-hourly-task.cmd`) | 1) Fetches data (PubMed, CDC, cancer, case reports, TCIA, curated). 2) Writes to DB and JSON. 3) Think + **agents** (surveillance reviewer, literature reviewer, synthesizer) write insights and **opportunities**. 4) Push to GitHub. See [AGENTS.md](./AGENTS.md). |
...
So **when you go to sleep**, the only thing that keeps going (if your PC stays on) is the **ingest task**: every 12 hours it collects new data and pushes it to GitHub. Nothing is reading that data and “thinking” about how to improve animal health – no analysis, no alerts, no suggested next steps.
...
1. **If your PC is on** and the scheduled task is set up:
   - **Every 12 hours** the task runs: ingest → DB + JSON → push to GitHub.
...
2. **If your PC is off or asleep**:
   - The task does **not** run. No ingest, no push. When you turn the PC back on, the next run is at the next scheduled time (every 12 hours).
...
1. **Keep doing** what it does: ingest every 12 hours (surveillance, literature, cancer, case data, imaging).
...
| Question | Answer |
|----------|--------|
| **What is it currently running?** | The **ingest task every 12 hours** (data collection + push to GitHub). The **dashboard** only runs if you started it with `npm start`. |
| **If I go to sleep, what will it do?** | If the PC stays on: **every 12 hours** it will ingest data and push to GitHub, and (with the new step) write **`memory/autonomous-insights.md`** with a short “what matters for animal health” summary. |
