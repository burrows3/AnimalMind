...
1. **If your PC is on** and the scheduled task is set up:
   - **Every 6 hours** the task runs: ingest → DB + JSON → push to GitHub.
...
2. **If your PC is off or asleep**:
   - The task does **not** run. No ingest, no push. When you turn the PC back on, the next run is at the next scheduled time (every 6 hours).
...
| Question | Answer |
|----------|--------|
| **What is it currently running?** | The **ingest task every 6 hours** (data collection + push to GitHub). The **dashboard** only runs if you started it with `npm start`. |
| **If I go to sleep, what will it do?** | If the PC stays on: **every 6 hours** it will ingest data and push to GitHub, and (with the new step) write **`memory/autonomous-insights.md`** with a short “what matters for animal health” summary. |
